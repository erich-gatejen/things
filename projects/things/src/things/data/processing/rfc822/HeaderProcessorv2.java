 ...
 * A call to process() will process a specific message, which will in turn call the other abstract methods for matches (or not-matched).  The object can
 * be reused all you want--so keep it around and process() over and over again.
 * <p>
 * This one properly calls headersComplete after the final header and before any terminating CF/LF (or whatever).  HOWEVER!  It may leave CRs and LFs from the header
 * in the InputStream.  Adding lookahead for that would be a giant pain in the ass and really slow things down on the other side.  So, any body processor needs to make sure 
 * it reads and writes these characters, just emits a CR/LF itself before eating any dangling header whitespace, or something in between.
 * <p>
 ...
 * EPG - Actually fixed - 1 APRIL 06
 * </pre> 
 */
public abstract class HeaderProcessorv2 extends InputStream  {
	...
	//private boolean 		terminatingPause;
	...
	// =========================================================================================
	// CONSTRUCTOR
	public HeaderProcessorv2() throws Throwable {
	 ...
	 * @throws Throwable
	 */
	abstract protected void headersComplete(OutputStream 	out) throws Throwable;
		...
		while (current >= 0) {
			...
			case LexicalTool.ALLOWED:
				...
				if (stateChart[currentColumn][current] == STATE_NONE) {
					...
					// extend the table?
					if (stateChart.length <= (nextColumn+1)) extendChartDeclaration();
	...
	public synchronized List<String> process(String messageId, InputStream input, OutputStream output) throws Throwable {
		...
		if (outs==null) {
			...
			while (!isDone()) {
				...
				// Empty the line in case a match didnt.
				deplete();
				
				// Are we done?
				if ((readState == LexicalTool.HP____SPECIAL_WALKING_DEAD)||(readState == LexicalTool.HP____SPECIAL_DEAD)) {
					if (!isComplete) {
						headersComplete(outs);	
						isComplete = true;
					}
				}
				...
				// Clear match state no matter what.
				currentColumn = STATE_NONE;
	...
	/**
	 * Get the header name.  It will assume it is US-ASCII.
	 * @return 
	 * @throws Throwable
	 */
	public String getHeaderName() throws Throwable {
		if (headerBufferLength>0)
			return new String(headerBuffer,0,headerBufferLength, "US-ASCII");
		else
			return "";
	}
	...
	public int read() throws IOException {
		while(true) {
			...
			case LexicalTool.HP_START:
				...
				if (teeCharacter>=0) {
					readState = LexicalTool.HEADER_READ_STATE_CHARTV2[teeCharacter][readState];
				} else {
			...
			case LexicalTool.HP_PAUSE_CRLFCR:
				//if (teeCharacter>=0) {
				//	unpauseState = LexicalTool.HEADER_READ_STATE_CHARTV2[teeCharacter][readState];
				//} else {
				//	unpauseState = LexicalTool.HP_CLOSURE;
				//}
				//terminatingPause = true;
				readState = LexicalTool.HP_CLOSURE;
				return -1;
			...
			case LexicalTool.HP_CLOSURE:
				readState = LexicalTool.HP____SPECIAL_WALKING_DEAD;
				return -1;
				
			case LexicalTool.HP____SPECIAL_WALKING_DEAD:
				readState = LexicalTool.HP____SPECIAL_DEAD; 
				if (teeCharacter>=0) return teeCharacter;
				return -1;
