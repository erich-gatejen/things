...
/**
 * General command service.  All commands will come over a push drain.
 * <br>
 * All commands will respond with a Receipt, a reason, and a number (the latter two as attributes).
 * <p>
 * It expects the following local properties:
 * <p>
 ...
 * </pre> 
 */
public class LocalCommandService extends Service implements PushDrain {
	...
	private LinkedBlockingQueue<Entity<LocalCommandInfo>>	inboundCommands;
	...
	public void executeThingsProcess() throws SystemException {
		...
		try {
			...
			// Loop it
			Entity<LocalCommandInfo> currentCommand;
			while(true) {
				currentCommand = inboundCommands.take();
				if (currentCommand==null) throw new Exception("Null Command object.");  // Just in case.
				myLogger.debug("Command dequeued: " + currentCommand.getID().toString());
	...
	public void constructThingsProcess() throws SystemException {
		...
		try {
			...
			// Create bridge queue
			inboundCommands = new LinkedBlockingQueue<Entity<LocalCommandInfo>>();
	...
	public Receipt postListener(Data		n) throws SystemException {
		...
		synchronized(serviceIsOn) {
			...
			try {
				...
				item = (Entity<LocalCommandInfo>)n;
				LocalCommandInfo info = item.getTypedThing();
	...
	private String	commandTagging(String info) {
		return "LocalCommandService:" + this.getProcessId() + ":" + served.getAndIncrement() + ":" + info;
	}
	...
	/**
	 * Process the command.  An exception coming out of here is a very bad thing.
	 */
	private void processCommand(Entity<LocalCommandInfo> commandData) throws Throwable { 
		
		LocalCommandInfo info =  null;
		Conduit responseConduit = null;
		LocalCommandResponder responder = null;
		Command c = null;
		
		// Create the response channel.  This is the unrecoverable part of the logic.  These exceptions will propigate.  We'll try to hold everything else.
		try {
			
			//Get the resposne conduit.
			info = commandData.getTypedThing();
			responseConduit = systemConduitController.tune(info.responseConduit,this.getProcessId());
			responder = new LocalCommandResponder(commandData, responseConduit, this.getProcessId(), ssi);
			
		} catch (Throwable t) {
			screech("Spurious exception while establishing response channel for a command. message=" + t.toString());
			throw t;
		}
		
		// Start holding exceptions.  At this stage, we are instantiating.
		try {
			
			// Build the command
			c = Command.instantiateResponse(info.commandName,responder);
			
		} catch (SystemException se) {
			screech("System Exception during command instantiation.  message=" + se.toString());
			responder.reportFailure(se.toStringCauses());
		} 
		...
		try {
			...
			if (this.isVerbose()) screech("System Exception during command execution.  message=" + se.toStringComplex());
			else screech("System Exception during command execution.  message=" + se.toString());
			responder.reportFailure(se.toStringCauses());
		} 
